...
Silnik::Silnik() : aktualnaTura(0), czyDzialaSymulacja(false) {
    //ustawianie zasobow
    miasto.getMenedzerZasobow().dodajZasob(TypZasobu::FOOD, 6);
    miasto.getMenedzerZasobow().dodajZasob(TypZasobu::GOLD, 10);
    miasto.getMenedzerZasobow().dodajZasob(TypZasobu::WOOD, 100);
    miasto.getMenedzerZasobow().dodajZasob(TypZasobu::STONE, 50);
    //mieszkancy
    miasto.dodajMieszkanca(std::make_unique<Rolnik>(40, 30, 100, &miasto.getMenedzerZasobow(), 10, 2));
    miasto.dodajMieszkanca(std::make_unique<Rolnik>(40, 30, 100, &miasto.getMenedzerZasobow(), 10));
    miasto.dodajMieszkanca(std::make_unique<Rolnik>(40, 30, 100, &miasto.getMenedzerZasobow(), 10));
    miasto.dodajMieszkanca(std::make_unique<Kupiec>(40, 30, 100, &miasto.getMenedzerZasobow(), 2));
    miasto.dodajMieszkanca(std::make_unique<Kupiec>(40, 30, 100, &miasto.getMenedzerZasobow(), 6));
    miasto.dodajMieszkanca(std::make_unique<Straznik>(40, 30, 100,&miasto, &miasto.getMenedzerZasobow(), 10, 1));
}
...
void Silnik::wykonajTure() {
    ...
    if (miasto.sprawdzWarunkiUpadku()) {
        ...
        std::cout << "=========================================" << std::endl;
        std::cout << "Przetrwane Tury: " << aktualnaTura << std::endl;
...
void Silnik::wyswietlRaport() const {
    ...
    std::cout << " Wydarzenie losowe: " << std::endl;
    std::cout << "=========================================" << std::endl;
}
